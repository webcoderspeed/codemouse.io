  ...
  try {
    ...
    if (event === 'installation') {
      if (payload.action === 'created') {
        ...
        // Send "app installed" email if we have the user
        if (inst.userId) {
    ...
    const prUrl: string          = pull_request.html_url ?? ''

    console.log(`[webhook] PR #${prNumber} opened in ${repoFullName}`)

    // ── 4. Check usage limits ─────────────────────────────────────────────
    const allowed = await isWithinLimit(installationId)

    if (!allowed) {
      const octokit = await getInstallationOctokit(installationId)
      await postPRComment(octokit, owner, repo, prNumber,
        `## 🐭 CodeMouse — Limit Reached\n\nYour free plan allows **30 reviews/month**. This PR won't be reviewed automatically.\n\n👉 [Upgrade to Pro](https://codemouse.io/pricing) for unlimited reviews — ₹599/month.\n\n*[CodeMouse](https://codemouse.io)*`
      )

      // Send limit-reached email to owner
      const inst = await getInstallation(installationId)
      if (inst?.userId) {
        const user = await getUserById(inst.userId.toString())
        if (user?.email) {
          sendLimitReachedEmail(user.email, user.name, inst.accountLogin)
            .catch(e => console.error('[mailer] limitReached email failed:', e))
        }
      }
      return NextResponse.json({ ok: true, skipped: 'limit reached' })
    }
    ...
    // ── 6. Run AI review ──────────────────────────────────────────────────
    const review = await reviewPullRequest(prTitle, prDesc, diff, repoFullName)
    ...
    // ── 9. Usage warning emails (80% threshold) ────────────────────────
    const inst = await getInstallation(installationId)
    if (inst && inst.plan === 'free' && inst.userId) {
      const pct = Math.round(((inst.reviewsUsed + 1) / inst.reviewsLimit) * 100)
      // Send warning at exactly 80% (allow ±1 review slack)
      if (pct >= 80 && pct < 90) {
        const user = await getUserById(inst.userId.toString())
        if (user?.email) {
          sendUsageWarningEmail(
            user.email, user.name,
            inst.reviewsUsed + 1, inst.reviewsLimit,
            inst.accountLogin
          ).catch(e => console.error('[mailer] usageWarning email failed:', e))
        }
      }
    }
